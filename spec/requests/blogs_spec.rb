...
RSpec.describe 'Blogs', type: :request do
  ...
  let!(:blogs) { create_list(:blog_content, 5, user:) }
  ...
  describe 'GET /index' do
    ...
    it do
      ...
      expect(res['blogs'].first.keys).to eq(%w[id title body])
    end
  end
  ...
  describe 'POST /create' do
    ...
    it do
      expect(response).to have_http_status(:ok)
      res = response.parsed_body
      expect(res.keys).to eq(%w[message blog])
      expect(res['blog'].keys).to eq(%w[id title body])
    end
  end
